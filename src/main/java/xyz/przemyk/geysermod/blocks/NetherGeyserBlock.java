...
public class NetherGeyserBlock extends GeyserBlock {
    ...
    public void randomTick(BlockState state, World worldIn, BlockPos pos, Random random) {
        ...
        if (worldIn instanceof ServerWorld) {
            ((ServerWorld) worldIn).spawnParticle(ParticleTypes.LAVA, (double) blockpos.getX() + 0.5D, (double) blockpos.getY() + 0.25D, (double) blockpos.getZ() + 0.5D, 200, 0.1D, 3.0D, 0.1D, 0.0D);
        }
